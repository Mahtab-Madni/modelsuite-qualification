...
const REVIEW_STATUS_CLASS = {
  ...
  "Request Revision": "status-badge-Needs-Revision",
};
    ...
    try {
      ...
      notifySuccess(
        status === "Request Revision"
          ? "Submission sent for revision"
          : `Submission ${status.toLowerCase()}d`,
      );
      onReviewed();
      onClose();
    } catch (err) {
      notifyError(err.response?.data?.message || "Review action failed");
    }
  };
      ...
      >
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div className="bg-bg-surface rounded-lg px-4 py-3.5 border border-border">
            <p className="text-[11px] font-semibold uppercase tracking-[0.5px] text-text-faint mb-1.5">
              Task
            </p>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              {task.status && (
                <span
                  className={`inline-block px-2 py-[2px] rounded-full text-[11px] font-medium ${TASK_STATUS_CLASS[task.status] || "status-badge-Open"}`}
                >
                  {task.status}
                </span>
              )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto">
              <span
                className={`inline-block px-2.5 py-[3px] rounded-full text-[11px] font-semibold ${REVIEW_STATUS_CLASS[submission.reviewStatus] || "status-badge-Submitted"}`}
              >
          ...
          <div className="flex gap-3 pt-1 border-t border-border mt-1">
            ...
            <button
              ...
              className="flex-1 py-2.5 bg-warning/10 text-warning border border-warning/30 rounded-lg text-sm font-semibold cursor-pointer hover:bg-warning/20 transition-all font-sans"
            >
              ↺ Request Changes
            </button>
            <button
