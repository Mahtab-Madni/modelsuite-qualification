...
const TaskCard = ({ task, showClaimButton = false, onClaimed }) => {
  const handleClaim = async () => {
    try {
      ...
      notifyError(err.response?.data?.message || "Failed to claim task");
    }
  };
  ...
  return (
    <div className="bg-bg-card border border-border rounded-xl p-5 flex flex-col gap-3 hover:border-border-light hover:-translate-y-0.5 transition-all cursor-default">
      ...
      <div className="flex items-start justify-between gap-2.5">
        ...
        {task.status && (
          <span
            className={`shrink-0 inline-block px-2.5 py-[3px] rounded-full text-[11px] font-semibold tracking-[0.3px] ${STATUS_CLASS[task.status] || ""}`}
          >
            {task.status}
          </span>
        )}
